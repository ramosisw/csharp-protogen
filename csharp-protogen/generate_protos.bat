...
echo.
SET "proto_path=%~dp0"
FOR /F "usebackq delims=*" %%F in (`dir /S /A /B *.proto`) DO (
    echo found %%~nF%%~xF
    @rem echo  --csharp_out "%%~dF%%~pF." "%%F" --grpc_out "%%~dF%%~pF." --plugin=protoc-gen-grpc=%PLUGIN%
    %PROTOC% --proto_path="%proto_path%." --csharp_out "%%~dF%%~pF." "%%F" --grpc_out "%%~dF%%~pF." --plugin=protoc-gen-grpc=%PLUGIN%
)
